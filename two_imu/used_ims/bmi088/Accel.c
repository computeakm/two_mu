...
uint8_t a_maxRangeBits;
double a_maxRangeReal;
uint8_t a_bwp;
uint8_t a_odr;
...
uint8_t ACCEL_SELF_TEST(void){
    ...
    // Setup
    ACCEL_SET_RANGE(ACCEL_RANGE_6G);
    ACCEL_SET_CONFIG(ACCEL_OSR_4, ACCEL_ODR_1600);
	  setRangeMem(ACCEL_RANGE_6G);
    // ACCEL_RELOAD_SETTINGS();
...
void ACCEL_RELOAD_SETTINGS(void){
    ...
    a_bwp = (rawData[0] >> 4 ); 
    a_odr = rawData[0] & 0x0f ; 

    rawData[1] = rawData[1] & 0x03  ; // Last 2
...
AccelError ACCEL_READ_ERROR(void){
    ...
    out.isFatal = val & 0x01;
    out.errorCode =(val & 0x1C)>> 2;
...
uint8_t ACCEL_READ_ACCEL_RED(void){
    uint8_t reday;
    select();
    readAddr(ADDR_INT_STAT_1, &reday, 1);
    unselect();
    if((reday&0x80)==0x80)
		{
			return 1;
...
void ACCEL_WRITE_PWR_ACTIVATE(void){
    select();
    writeAddr(ADDR_ACC_PWR_CONF, ACCEL_PWR_ACTIVE);
    unselect();
}
void ACCEL_WRITE_PWR_SUSPEND(void){
    select();
    writeAddr(ADDR_ACC_PWR_CONF, ACCEL_PWR_SUSPEND);
    unselect();
}
void ACCEL_WRITE_ACCEL_ENABLE(void){
    select();
    writeAddr(ADDR_ACC_PWR_CTRL, ACCEL_ACCEL_ENABLED);
    unselect();
}
void ACCEL_WRITE_ACCEL_DISABLE(void){
    select();
...
uint16_t ACCEL_READ_FIFO_LEN(void){
    uint8_t rawData[2];
    select();
...
AccelDataBuffer ACCEL_READ_FIFO(void){
...
static Vector3 parseRawUInts(uint8_t* rawVals){
    ...
    x = ((int16_t)(rawVals[1]*256 + rawVals[0])) * (2 << ACCEL_RANGE_6G);
    y = ((int16_t)(rawVals[3]*256 + rawVals[2])) * (2 << ACCEL_RANGE_6G);
    z = ((int16_t)(rawVals[5]*256 + rawVals[4])) * (2 << ACCEL_RANGE_6G);
